...
TYPED_TEST_P(TcTxnTest, BadFileCreation)
{
	...
	const std::vector<std::string> paths{
		"bad-creation/good/a", "bad-creation/good/b",
...
TYPED_TEST_P(TcTxnTest, BadOpenWithTrunc)
{
	...
	for (int i = 0; i < n; ++i) {
		viov2file(&v1[i], &files[i], 0, datasize,
			  getRandomBytes(datasize));
	}
	...
	/* check state */
	for (int i = 0; i < paths.size(); ++i) {
		EXPECT_TRUE(posix_integrity(this->posix_base, paths[i],
					    v1[i].data, datasize))
		    << paths[i];
	}
...
TYPED_TEST_P(TcTxnTest, BadCreationWithExisting)
{
	...
	const std::vector<std::string> paths1{ "bad-creation3/c",
					       "bad-creation3/d" };
	const std::vector<std::string> paths2{
		"bad-creation3/a", "bad-creation3/b",  "bad-creation3/c",
		"bad-creation3/d", "bad-creation3/no", "bad-creation3/f"
	};
	...
	/* create files in paths2[] - this will fail and rollback, but we expect
	 * that files in paths1 won't be removed */
	...
	for (int i = 0; i < paths1.size(); ++i) {
		EXPECT_TRUE(posix_exists(this->posix_base, paths1[i]))
		    << paths1[i];
	}
...
TYPED_TEST_P(TcTxnTest, BadRemoveCheckContent)
{
	...
	for (int i = 0; i < n; ++i) {
		viov2file(&iov[i], &files[i], 0, datasize,
			  getRandomBytes(datasize));
	}
...
TYPED_TEST_P(TcTxnTest, BadLink)
{
	...
	const std::vector<std::string> src{ "bad-link/a", "bad-link/b",
					    "bad-link/c", "bad-link/d",
					    "bad-link/e", "bad-link/f" };
	const std::vector<std::string> dest{ "bad-link/1", "bad-link/2",
					     "bad-link/3", "bad-link/d",
					     "bad-link/5", "bad-link/6" };
	...
	for (int i = 0; i < dest.size(); ++i) {
		EXPECT_FALSE(posix_exists(this->posix_base, dest[i]))
		    << dest[i];
	}
...
TYPED_TEST_P(TcTxnTest, BadWrite)
{
	...
	ASSERT_TRUE(v2);
	for (int i = 0; i < n; ++i) {
		viov2file(&v2[i], &files[i], 0, datasize,
			  getRandomBytes(datasize));
	}
...
{
	const std::vector<std::string> paths{ "PRE-1-open-excl.txt",
					      "PRE-2-open-excl.txt",
					      "PRE-3-open-excl.txt",
					      "PRE-4-open-excl.txt" };
	vfile *files;
...
{
	const std::vector<std::string> paths{ "PRE-1-open.txt",
					      "PRE-2-open.txt",
					      "PRE-3-open.txt",
					      "PRE-4-open.txt" };
	vfile *files;
...
{
	const std::vector<std::string> paths{ "TcTxnTest-TestFileDesc1.txt",
