    ...
    const size_t iteration_count)
{
	for (size_t t = 0; t < iteration_count; t++) {
		vfile *files =
		    tc::vec_open_simple(paths, O_CREAT | O_RDWR, 0666);
    ...
    const size_t iteration_count)
{
	...
	for (size_t t = 0; t < iteration_count; t++) {
		for (size_t i = 0; i < item_count; ++i) {
			viov2path(&v[i], paths[i].c_str(), 0,
				  sizeof(write_data[i]), (char *)write_data[i]);
		}
		EXPECT_OK(vec_write(v.data(), item_count, true));
	}
}
    ...
    const bool &writers_finished)
{
	...
	while (!writers_finished) {
		...
		for (size_t i = 0; i < item_count; ++i) {
			viov2path(&v[i], paths[i].c_str(), 0, max_item_size,
				  buffer[i]);
		}
		...
		for (const auto &write_set : write_sets) {
			is_expected |= (memcmp(buffer.data(), write_set.data(),
					       buflen) == 0);
		}
...
TYPED_TEST_P(TcLockTest, SerializabilityRW)
{
	...
	const std::vector<std::array<char[max_write_size], num_writes> >
	    write_sets{ std::array<char[max_write_size], num_writes>{
			    "apple", "banana", "mango" },
			std::array<char[max_write_size], num_writes>{
	...
	for (size_t i = 0; i < FLAGS_nwriter_threads; i++) {
		const auto &write_set =
		    write_sets[std::rand() % write_sets.size()];
		writer_threads.emplace_back(
		    execute_write<num_writes, max_write_size>, std::ref(paths),
	...
	for (size_t i = 0; i < FLAGS_nreader_threads; i++) {
		reader_threads.emplace_back(
		    execute_read<num_writes, max_write_size>, std::ref(paths),
		    std::ref(write_sets), std::ref(writers_finished));
	}
...
TYPED_TEST_P(TcLockTest, SerializabilityFileCR)
{
	...
	/* each thread performs a write with values in the set */
	std::array<char[max_write_size], num_writes> write_set{ "apple",
								"banana",
								"mango" };
	...
	create_remove_thread = std::thread(
	    execute_create_remove_file<num_writes, max_write_size>,
	    std::ref(paths), std::ref(write_set), FLAGS_nwriter_iterations);

	/* use posix api's to verify either all files exist or none */
	for (size_t i = 0; i < FLAGS_nreader_threads; i++) {
		open_threads.emplace_back(
		    execute_posix_path_exists, std::ref(paths),
		    std::ref(this->nfs_base), std::ref(writers_finished));
	}
...
TYPED_TEST_P(TcLockTest, SerializabilityDirCR)
{
	...
	create_remove_thread =
	    std::thread(execute_create_remove_dir, std::ref(paths),
...
TYPED_TEST_P(TcLockTest, MultipleWritesOnSameFile)
{
	...
	while (completed_files < num_writes) {
		for (size_t i = 0; i < num_writes; ++i) {
			if (offsets[i] < total_data_size) {
				/* random write size */
				size_t write_size =
				    std::min((std::rand() % max_write_size) + 1,
					     total_data_size - offsets[i]);
				struct viovec iovec;
				viov2path(&iovec, paths[i].c_str(), offsets[i],
					  write_size, &data[offsets[i]]);
...

	{
		...
		for (size_t i = 0; i < num_writes; ++i) {
			viov2path(&v[i], paths[i].c_str(), 0, total_data_size,
				  buffer[i]);
		}
