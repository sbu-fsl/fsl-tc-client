...
static std::vector<Slice> tc_get_path_components(Slice path)
{
	...
	path.trim('/');
	int beg = 0;
	unsigned int end = 0;   // component within [beg, end)
	while (end < path.size()) {
...
static int tc_path_append_impl(buf_t *pbuf, slice_t comp)
{
	...
	if (comp.size > (size_t)buf_remaining(pbuf)) {
		return -1;
	}
	if (pbuf->size != 0) {
...
int tc_path_rebase_s(slice_t base, slice_t path, buf_t *pbuf)
{
	...
	int result_size = 0;
	for (unsigned int i = l; i < base_comps.size(); ++i) {
		relative_comps.push_back("..");
		result_size += 2;
	}
	for (unsigned int j = l; j < path_comps.size(); ++j) {
	...
	size_t size = 0;
	for (unsigned int i = 0; i < relative_comps.size(); ++i) {
